...
const LudoGrid = () => {
    // Colores basados en la imagen "LUDO LITERARIO"
    ...
    const Cell = ({ color, icon, className = "" }: any) => (
        <div 
            className={`w-full h-full border border-black/10 flex items-center justify-center text-lg ${className}`} 
    ...
    const Base = ({ color, icon }: any) => (
        <div className="col-span-6 row-span-6 border-[clamp(2px,0.5vh,6px)] border-[#4a2e1d] rounded-[clamp(0.5rem,2vh,2rem)] p-[clamp(2px,1vh,16px)] flex items-center justify-center shadow-inner" style={{ backgroundColor: color }}>
            <div className="w-full h-full bg-white/10 rounded-xl flex items-center justify-center text-[clamp(1.5rem,6vh,4rem)] drop-shadow-2xl">
                {icon}
            </div>
        </div>
    );
    ...
    return (
        <div className="aspect-square h-[min(60vh,85vw)] max-w-[95vw] bg-[#e8dfc5] border-[clamp(8px,1.5vh,16px)] border-[#3d2616] rounded-[clamp(1rem,3vh,3rem)] shadow-[0_40px_100px_rgba(0,0,0,0.6)] p-[clamp(8px,2vh,24px)] grid grid-cols-15 grid-rows-15 relative overflow-hidden ring-[clamp(2px,0.5vh,8px)] ring-[#4a2e1d]/30">
            {/* Textura de pergamino sutil con gradientes */}
            <div className="absolute inset-0 pointer-events-none opacity-40 bg-[radial-gradient(circle_at_20%_30%,_rgba(255,255,255,0.4)_0%,_transparent_50%),_radial-gradient(circle_at_80%_70%,_rgba(0,0,0,0.1)_0%,_transparent_50%)]"></div>
            
            {/* Bases */}
            <Base color={green} icon="🏰" />
            <div className="col-span-3 row-span-6 grid grid-cols-3 grid-rows-6 p-1 gap-[clamp(2px,0.2vh,6px)]">
                {Array(18).fill(0).map((_, i) => <Cell key={i} color={i % 3 === 1 && i > 0 ? green : parchment} className="rounded-sm shadow-sm" />)}
            </div>
            <Base color={red} icon="🐲" />

            {/* Fila del medio */}
            <div className="col-span-6 row-span-3 grid grid-cols-6 grid-rows-3 p-1 gap-[clamp(2px,0.2vh,6px)]">
                {Array(18).fill(0).map((_, i) => <Cell key={i} color={i >= 6 && i < 11 ? blue : parchment} className="rounded-sm shadow-sm" />)}
            </div>
            <div className="col-span-3 row-span-3 bg-gradient-to-br from-[#911d1d] via-[#1d6b2f] to-[#cc8a16] border-[clamp(2px,0.8vh,8px)] border-[#4a2e1d] flex items-center justify-center p-[clamp(2px,0.5vh,8px)] rounded-xl shadow-lg ring-2 ring-white/10">
                <div className="w-full h-full bg-[#f3edd7] rounded-lg flex items-center justify-center text-[clamp(1rem,4vh,2.5rem)] shadow-inner border-2 border-[#4a2e1d]/20">🖋️</div>
            </div>
            <div className="col-span-6 row-span-3 grid grid-cols-6 grid-rows-3 p-1 gap-[clamp(2px,0.2vh,6px)]">
                {Array(18).fill(0).map((_, i) => <Cell key={i} color={i >= 7 && i < 12 ? red : parchment} className="rounded-sm shadow-sm" />)}
            </div>

            {/* Bases Inferiores */}
            <Base color={blue} icon="🚢" />
            <div className="col-span-3 row-span-6 grid grid-cols-3 grid-rows-6 p-1 gap-[clamp(2px,0.2vh,6px)]">
                {Array(18).fill(0).map((_, i) => <Cell key={i} color={i % 3 === 1 && i < 15 ? blue : parchment} className="rounded-sm shadow-sm" />)}
            </div>
            <Base color={yellow} icon="🕯️" />
        </div>
    );
};
            ...
            <div className="relative inline-block">
                ...
                {players.map((player) => {
                    ...
                    if (isLudo) {
                        ...
                        const progress = Math.min(player.current_position / (totalQuestions || 1), 1);
                    ...
                    return (
                        <div 
                            key={player.id} 
                            className="absolute transition-all duration-1000 ease-out z-30 transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center" 
                            style={{ left: `${coordinate.x}%`, top: `${coordinate.y}%` }}
                        >
                            <span className="bg-black/80 text-white text-[clamp(8px,1vh,12px)] px-2 py-0.5 rounded-full mb-1 font-bold whitespace-nowrap border border-white/20 shadow-lg">{player.player_name}</span>
                            <div className="w-[clamp(2.5rem,6vh,4.5rem)] h-[clamp(2.5rem,6vh,4.5rem)] rounded-full border-[clamp(2px,0.4vh,4px)] border-white shadow-2xl overflow-hidden bg-white hover:scale-125 transition-transform">
