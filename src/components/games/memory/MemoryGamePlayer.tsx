...
export default function MemoryGamePlayer({
    ...
    const attemptsPerPairRef = useRef<Map<string, number>>(new Map());
    const hasTimedOutRef = useRef(false);
    const matchedRef = useRef<string[]>([]);

    // Sincronizar ref para usarla en effectos sin stale closure
    ...
    useEffect(() => {
        ...
        questions.forEach((q) => {
            ...
            const baseId = q.id;
            generatedCards.push({ id: `${baseId}_q`, content: q.question_text, type: "question", pairId: baseId });
            generatedCards.push({ id: `${baseId}_a`, content: correctAnswerText, type: "answer", pairId: baseId });
        });

        setCards(generatedCards.sort(() => Math.random() - 0.5));
        attemptsPerPairRef.current = new Map();
        hasTimedOutRef.current = false;

        const timer = setTimeout(() => setInitialAnimation(false), 1500);
        return () => clearTimeout(timer);
    }, [questions]);

    // 2. Bloquear tablero cuando el tiempo se acaba
    useEffect(() => {
        if (timeLeft === 0 && questionDuration > 0 && !isFinished && !hasTimedOutRef.current && cards.length > 0) {
            hasTimedOutRef.current = true;
            setLockBoard(true);
            setTimedOut(true);
            onSubmit(0, matchedRef.current.length);
        }
    }, [timeLeft, questionDuration, isFinished, cards.length, onSubmit]);

    // Algoritmo de cuadrícula optimizado para landscape
    ...
    if (totalCards === 10) { cols = 5; rows = 2; }
    ...
    else if (totalCards === 14) { cols = 7; rows = 2; }
        ...
        // Forzar proporción landscape ~1.6:1 (más columnas que filas)
        cols = Math.ceil(Math.sqrt(totalCards * 1.6));
    ...
    // 3. Calcular tamaño de cartas dinámicamente
    const calculateCardSize = useCallback(() => {
        ...
        const gapSize = 10;
        const availW = containerW - gapSize * (cols - 1);
        const availH = containerH - gapSize * (rows - 1);
    ...
    // 4. Lógica de selección con guards corregidos
    const handleCardClick = (index: number) => {
        if (lockBoard || initialAnimation || matched.includes(cards[index].pairId)) return;
        // Evitar doble click en la misma carta (incluyendo race conditions)
        ...
        if (newFlipped.length === 2) {
            ...
            if (card1.pairId === card2.pairId && card1.type !== card2.type) {
                // ¡Pareja correcta! — puntos según intentos previos en esa pareja
                const failedAttempts = attemptsPerPairRef.current.get(card1.pairId) ?? 0;
                let pointsToAdd = failedAttempts === 0 ? 150 : failedAttempts === 1 ? 100 : 50;

                const currentPairsFound = matchedRef.current.length + 1;
                setMatched(prev => [...prev, card1.pairId]);
                setMatchOrder(prev => [...prev, card1.pairId]);
                setLastMatchedPair(card1.pairId);
                setTimeout(() => setLastMatchedPair(null), 400); // Sincronizado con animación
                setFlipped([]);
                setLockBoard(false);
                ...
                // Fallo — registrar intento fallido para cada pareja expuesta
                const pairId1 = card1.pairId;
                const pairId2 = card2.pairId;
                attemptsPerPairRef.current.set(pairId1, (attemptsPerPairRef.current.get(pairId1) ?? 0) + 1);
                if (pairId1 !== pairId2) {
                    attemptsPerPairRef.current.set(pairId2, (attemptsPerPairRef.current.get(pairId2) ?? 0) + 1);
                }
    ...
    return (
        <div className="flex flex-col items-center w-full h-full p-3 sm:p-4 relative">
            ...
            >
                ...
                {cardSize.w > 0 && (
                    ...
                    >
                        {cards.map((card, idx) => {
                            ...
                            // Cartas emparejadas más antiguas se atenúan ligeramente
                            const matchPosition = isMatched ? matchOrder.indexOf(card.pairId) : -1;
                            const isOldMatch = isMatched && matchPosition < matchOrder.length - 2;
                ...
                {/* Overlay: ¡Completado! */}
                {isFinished && (
                    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/70 backdrop-blur-md rounded-[2rem] animate-fade-in">
                        <div className="text-center px-6">
                            <div className="text-7xl mb-4 animate-bounce">🎉</div>
                            <h2 className="text-3xl font-black text-white mb-2 tracking-tight">¡Completado!</h2>
                            <p className="text-emerald-400 font-bold text-lg">Encontraste todas las parejas</p>
                            <div className="mt-4 flex items-center justify-center gap-3">
                                <div className="bg-white/10 border border-white/20 rounded-2xl px-5 py-3 text-center">
                                    <div className="text-[10px] font-black text-white/50 uppercase tracking-widest">Intentos</div>
                                ...
                                <div className="bg-emerald-500/20 border border-emerald-400/40 rounded-2xl px-5 py-3 text-center">
                                    <div className="text-[10px] font-black text-emerald-400/60 uppercase tracking-widest">Parejas</div>
                                    <div className="text-2xl font-black text-emerald-300">{questions.length}/{questions.length}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {timedOut && !isFinished && (
                    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/70 backdrop-blur-md rounded-[2rem] animate-fade-in">
                        <div className="text-center px-6">
                            ...
                            <h2 className="text-3xl font-black text-rose-400 mb-2 tracking-tight">¡Tiempo!</h2>
                            <p className="text-white/60 font-bold text-lg">Encontraste {matched.length} de {questions.length} parejas</p>
                        </div>
                    </div>
                )}
            </div>
